<?php $rootPath = '../'; $page = "careerlaunch"; include('../parts/head.php'); ?>
<body>
    <?php include('../parts/navbar.php'); ?>

    <section class="above-the-fold">
        <div class="container">
            <img src="../images/logos/techlaunch-blackbg.svg" alt="Techlaunch logo" class="logo">
        </div>
    </section>

    <section class="benefits">
        <div class="container">
            <h2 class="section-title">benefits of techlaunch</h2>
            <p>TechLaunch at FVI is committed to providing you a quality education in a lifelong, rewarding career. We take pride in offering first-class training and the guidance, tools and support needed to prepare you for the job you want.</p>
            <p>We are real developers dedicated to giving you a quality education to prepare you to get a job. We don't waste your time making you jump through hoops or taking a bunch of boring classes you don't need.</p>
        </div>
    </section>

    <section class="testimonial-1">
        <div class="left">
            <img src="../images/people/people-2.png" alt="">
        </div>
        <div class="right">
            <p>Hialeah sweetwater quis guy MiMo, fugiat velit ut sint dassitttt little haiti. Design district fire bird road ullamco UM Miami time Marlins id traffic qui cupidatat pariatur. Souwesera literally exercitation ut, domino gorda que fancy Mr. 305/Worldwide reprehenderit dolor park west coral way. Ocean drive tiki tiki music gorda, power love hour flagami flaca ut kendall dolore Pepe Billete abuelita.</p>
        </div>
    </section>

    <section class="how-we-can-help">
        <div class="container">
            <h2 class="section-title">how we can help (career services team)</h2>
            <p>Miami ipsum D. Wade amet labore voluptate park west ipsum yahs culpa minim lonche irure mission bro metromover. Ricer adipisicing veniam laborum excepteur guateber basel miami art basel magna vizcaya. Haslem wynwood Pitbull no yeah, yeah no civic center power 96 bulykkk. Bulykkk eu dolphin mall booty bass deserunt. North beach proident duis 826 quis pariatur quesito Heat flaca ea Pepe Billete critical mass. Ea cillum literally, ut fontainebleau deserunt flaca dolore labore sed officia abuelita doralzuela PLP. Nisi gorda exercitation, 826 ricer reprehenderit fontainebleau.</p>
            <p>Cafe llave Pitbull enim lorem, Pepe Billete cafecito souwesera eiusmod critical mass caja china. Ventanita flagler elit kendall commodo nulla, abuelita dolore gorda deserunt supposably occaecat reprehenderit. Tempor fontainebleau besito, voluptate tiki tiki music minim veniam est gramps duis. LIV tempor omni JWB miami, pariatur basel pastelitos croquetas dale consequat nisi pops porter duis.</p>
        </div>
    </section>

    <section class="testimonial-2">
        <div class="left">
            <img src="../images/people/people-5.png" alt="">
        </div>
        <div class="right">
            <p>Bulykkk Haslem aute vizcaya dolore dolore cillum ut non metromover edgewater PLP. Guateber UM domino officia, little haiti mollit coral way LIV no yeah, yeah no bulykkk lorem dolphin mall. Azucar ut flaca, vizcaya doralzuela supposably 305 till I die aliqua ut little haiti. Voluptate girl design district, de pinga la 20 y 20 mission bro laboris que fancy promoter friend lummus park abuelita omni.</p>
        </div>
    </section>

    <section class="hiring-partners">
        <div class="container">
            <h2 class="section-title">hiring partners info</h2>
            <p>Bulykkk buena vista FIU, fontainebleau north beach culpa west flagler quis ño omni coconut grove art deco mollit labore miami springs. Enim aliqua kendall, cafecito wynwood lorem ipsum laboris the roads. Design district non deserunt miami. Dolore ea super, flagler cafe con leche botanica FIU proident souwesera wynwood et critical mass. FIU ex bfe pastelitos. Que fancy yahs metromover ya tu sabes.</p>
            <p>Promoter friend gorda fugiat cupidatat commodo little havana aute dassitttt enim upper east side Miami time abuelita cocowalk in. Doralzuela eu Allapattah, collins ave sweetwater pata sucia kendall Haslem literally culpa. Dolore in que pena cafe bustelo cafe con leche non bird road omni et. Marlins casa de carajo dolor eiusmod.</p>
        </div>
    </section>

    <section class="testimonial-3">
        <div class="left">
            <img src="../images/people/person-holding-computer.png" alt="">
        </div>
        <div class="right">
            <p>Dolore cillum ut non metromover edgewater PLP. Guateber UM domino officia, little haiti mollit coral way LIV no yeah, yeah no bulykkk lorem dolphin mall. Azucar ut flaca, vizcaya doralzuela supposably 305 till I die aliqua ut little haiti. Voluptate girl design district, de pinga la 20 y 20 mission bro laboris que fancy promoter friend lummus park abuelita omni.</p>
        </div>
    </section>

    <section class="hiring-partners">
        <div class="container">
            <h2 class="section-title">hiring partners info</h2>
            <p>Bulykkk buena vista FIU, fontainebleau north beach culpa west flagler quis ño omni coconut grove art deco mollit labore miami springs. Enim aliqua kendall, cafecito wynwood lorem ipsum laboris the roads. Design district non deserunt miami. Dolore ea super, flagler cafe con leche botanica FIU proident souwesera wynwood et critical mass. FIU ex bfe pastelitos. Que fancy yahs metromover ya tu sabes.</p>
            <p>Promoter friend gorda fugiat cupidatat commodo little havana aute dassitttt enim upper east side Miami time abuelita cocowalk in. Doralzuela eu Allapattah, collins ave sweetwater pata sucia kendall Haslem literally culpa. Dolore in que pena cafe bustelo cafe con leche non bird road omni et. Marlins casa de carajo dolor eiusmod.</p>
        </div>
    </section>

    <section class="banner"></section>

    <section class="internships">
        <div class="container">
            <h2 class="section-title">internships</h2>
            <p>Bulykkk buena vista FIU, fontainebleau north beach culpa west flagler quis ño omni coconut grove art deco mollit labore miami springs. Enim aliqua kendall, cafecito wynwood lorem ipsum laboris the roads. Design district non deserunt miami. Dolore ea super, flagler cafe con leche botanica FIU proident souwesera wynwood et critical mass. FIU ex bfe pastelitos. Que fancy yahs metromover ya tu sabes.</p>
            <p>Promoter friend gorda fugiat cupidatat commodo little havana aute dassitttt enim upper east side Miami time abuelita cocowalk in. Doralzuela eu Allapattah, collins ave sweetwater pata sucia kendall Haslem literally culpa. Dolore in que pena cafe bustelo cafe con leche non bird road omni et. Marlins casa de carajo dolor eiusmod.</p>
        </div>
    </section>

    <section class="testimonial-4">
        <div class="left">
            <img src="../images/people/students.jpeg" alt="">
        </div>
        <div class="right">
            <p>Dolore cillum ut non metromover edgewater PLP. Guateber UM domino officia, little haiti mollit coral way LIV no yeah, yeah no bulykkk lorem dolphin mall. Azucar ut flaca, vizcaya doralzuela supposably 305 till I die aliqua ut little haiti. Voluptate girl design district, de pinga la 20 y 20 mission bro laboris que fancy promoter friend lummus park abuelita omni.</p>
        </div>
    </section>

    <section class="success-story">
        <div class="container">
            <h2 class="section-title">featured success story</h2>
            <p>Promoter friend gorda fugiat cupidatat commodo little havana aute dassitttt enim upper east side Miami time abuelita cocowalk in. Doralzuela eu Allapattah, collins ave sweetwater pata sucia kendall Haslem literally culpa. Dolore in que pena cafe bustelo cafe con leche non bird road omni et. Marlins casa de carajo dolor eiusmod.</p>
            <p>Bulykkk buena vista FIU, fontainebleau north beach culpa west flagler quis ño omni coconut grove art deco mollit labore miami springs. Enim aliqua kendall, cafecito wynwood lorem ipsum laboris the roads. Design district non deserunt miami. Dolore ea super, flagler cafe con leche botanica FIU proident souwesera wynwood et critical mass. FIU ex bfe pastelitos. Que fancy yahs metromover ya tu sabes.</p>
            <p>Little havana aute dassitttt enim upper east side Miami time abuelita cocowalk in. Doralzuela eu Allapattah, collins ave sweetwater pata sucia kendall Haslem literally culpa. Dolore in que pena cafe bustelo cafe con leche non bird road omni et. Marlins casa de carajo dolor eiusmod.</p>
        </div>
    </section>

    <section class="testimonial-5">
        <div class="left">
            <img src="../images/people/suit-guy.png" alt="">
        </div>
        <div class="right">
            <p>Dolore cillum ut non metromover edgewater PLP. Guateber UM domino officia, little haiti mollit coral way LIV no yeah, yeah no bulykkk lorem dolphin mall. Azucar ut flaca, vizcaya doralzuela supposably 305 till I die aliqua ut little haiti. Voluptate girl design district, de pinga la 20 y 20 mission bro laboris que fancy promoter friend lummus park abuelita omni.</p>
        </div>
    </section>

    <section class="reviews">
        <div class="container">
            <h2 class="section-title">reviews</h2>
            <p>Promoter friend gorda fugiat cupidatat commodo little havana aute dassitttt enim upper east side Miami time abuelita cocowalk in. Doralzuela eu Allapattah, collins ave sweetwater pata sucia kendall Haslem literally culpa. Dolore in que pena cafe bustelo cafe con leche non bird road omni et. Marlins casa de carajo dolor eiusmod.</p>
            <ul>
                <li>Dolore ea super, flagler cafe con leche botanica FIU proident souwesera</li>
                <li>Allapattah, collins ave sweetwater pata sucia kendall Haslem literally culpa</li>
                <li>Miami springs coconut grove croquetica calle ocho</li>
            </ul>
        </div>
    </section>

    <section class="student-projects">
        <div class="container">
            <h2 class="section-title">student projects</h2>
            <p>Promoter friend gorda fugiat cupidatat commodo little havana aute dassitttt enim upper east side Miami time abuelita cocowalk in. Doralzuela eu Allapattah, collins ave sweetwater pata sucia kendall Haslem literally culpa. Dolore in que pena cafe bustelo cafe con leche non bird road omni et. Marlins casa de carajo dolor eiusmod. Bulykkk buena vista FIU, fontainebleau north beach culpa west flagler quis ño omni coconut grove art deco mollit labore miami springs. Enim aliqua kendall, cafecito wynwood lorem ipsum laboris the roads. Design district non deserunt miami. Dolore ea super, flagler cafe con leche botanica FIU proident souwesera wynwood et critical mass. FIU ex bfe pastelitos. Que fancy yahs metromover ya tu sabes.</p>
            <p>Little havana aute dassitttt enim upper east side Miami time abuelita cocowalk in. Doralzuela eu Allapattah, collins ave sweetwater pata sucia kendall Haslem literally culpa. Dolore in que pena cafe bustelo cafe con leche non bird road omni et. Marlins casa de carajo dolor eiusmod.</p>
            <p>Design district pata sucia art basel bfe, irregardless girl jackson memorial fire upper east side consectetur basel cafe llave literally elit. Ricer JWB velit, anim cocowalk officia pariatur girl calle ocho est the roads consectetur super.</p>
        </div>
    </section>

    <?php $rootPath = '../'; include('../parts/footer.php') ?>

    <script src="../js/bundle.js"></script>
</body>
</html>
